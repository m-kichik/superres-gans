...
from tools import vanilla_gen_step, vanilla_discr_step
...
def main():
    ...
    G = SRResNet(scale_factor=2).to(DEVICE)
    D = Discriminator().to(DEVICE)
    
    G_optim = torch.optim.RMSprop(G.parameters(), lr=2e-5)
    D_optim = torch.optim.RMSprop(D.parameters(), lr=2e-5)
